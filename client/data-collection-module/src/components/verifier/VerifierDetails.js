...
const VerifierDetails = () => {
  ...
  const verifierInfo = useSelector(state => state.auth);
  const { userId, email, role } = verifierInfo;
  const [fullName, setFullName] = useState(verifierInfo.fullName);

  const handleEdit = async () => {
    try {
      const response = await api.put(`/verifier/updateVerifierFullname/${userId}`, { fullName });
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center py-8">
      <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-lg border border-gray-200">
        <div className="flex flex-col items-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-800">Verifier Details</h1>
        </div>
